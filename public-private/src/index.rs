...
impl Index {
    ...
    pub fn add_deps<R: RangeBounds<(u32, u32, u32)>>(
        ...
        let index_deps = self
            ...
            .or_default();
        for (p, pv, r) in deps {
            index_deps.insert(String::from(*p), (pv.clone(), range_from_bounds(r)));
        }
    }
}
...
#[cfg(test)]
pub mod tests {
    use super::Privacy::{Private, Public};
    use super::*;
    ...
    fn index_creation() {
        ...
        index.add_deps("a", (1, 0, 0), &[("b", Public, (1, 0, 0)..)]);
        index.add_deps("a", (2, 0, 0), &[("c", Private, ..)]);
        index.add_deps("b", (1, 0, 0), &[("d", Public, ..(4, 0, 0))]);
    }
}
